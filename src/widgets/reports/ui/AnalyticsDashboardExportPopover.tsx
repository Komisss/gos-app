...
type Props = {
  dashboardFilters: AnalyticsDashboardPayload;
...
export function AnalyticsDashboardExportPopover({
  ...
  const [filters, setFilters] = useState<AnalyticsDashboardPayload>(() =>
    createEmptyExportFilters(),
  );

  const exportMutation = useMutation({
    mutationFn: () =>
      createAnalyticsDashboardExport({
  ...
  function updateFilters(patch: Partial<AnalyticsDashboardPayload>) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[min(920px,calc(100vw-3rem))] p-4">
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Экспорт дашборда</h3>
            <p className="mt-1 text-xs text-slate-500">
              Фильтры экспорта открываются пустыми. Можно заполнить их вручную или перенести текущие
              фильтры дашборда.
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-between gap-2 border-t border-slate-200 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="outline" onClick={() => setFilters(dashboardFilters)}>
                Перенести фильтры из дашборда
              </Button>
...
function createEmptyExportFilters(): AnalyticsDashboardPayload {
  return {
    date_from: '',
    date_to: '',
