...
import { createReportsByOrgUnitsExport } from '@/entities/export/api/exports';
import type {
  ExportCreateResponse,
  ReportsByOrgUnitsExportFilters,
...
type Props = {
  tableFilters: ReportsByOrgUnitsPayload;
  ...
  taskStatusOptions: Array<{ value: ReportAnalyticsTaskStatus; label: string }>;
...
export function ReportsByOrgUnitsExportPopover({
  tableFilters,
  regionOptions,
  orgUnitOptions,
  taskOptions,
  ...
  const [filters, setFilters] = useState<ReportsByOrgUnitsExportFilters>(() =>
    createEmptyExportFilters(),
  );

  const exportMutation = useMutation({
    mutationFn: () =>
      createReportsByOrgUnitsExport({
        exportType: 'analytics_by_org_units',
        format: 'xlsx',
        filters,
        columns: orgUnitsExportColumns,
        asyncMode: true,
      }),
    onSuccess: (job) => {
      onExportStarted(job);
  ...
  function updateFilters(patch: Partial<ReportsByOrgUnitsExportFilters>) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[min(980px,calc(100vw-3rem))] p-4">
          ...
          <div className="grid max-h-[62vh] gap-4 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-4">
            ...
            <MultiSelect
              ...
              onChange={(task_statuses) =>
                updateFilters({ task_statuses: task_statuses as ReportAnalyticsTaskStatus[] })
              }
            />
            <MultiSelect
              label="Тип отчета"
            ...
            <FilterSelect
              ...
              options={[{ value: 'not_completed_rate', label: 'Доля невыполненных' }]}
...
function createEmptyExportFilters(): ReportsByOrgUnitsExportFilters {
  return {
    date_from: '',
    date_to: '',
    ...
    report_types: [],
    assignment_statuses: [],
    include_inactive_org_units: false,
...
function toExportFilters(filters: ReportsByOrgUnitsPayload): ReportsByOrgUnitsExportFilters {
  const { page: _page, page_size: _pageSize, org_unit_depth, ...exportFilters } = filters;
  return {
    ...exportFilters,
    org_unit_depth,
  };
