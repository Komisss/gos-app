...
const tableColumns: Array<{ key: keyof ReportsByUserItem; label: string; kind?: 'boolean' | 'date' | 'number' }> = [
  { key: 'task_id', label: 'ID задачи', kind: 'number' },
  { key: 'task_name', label: 'Задача' },
  { key: 'report_id', label: 'ID отчета', kind: 'number' },
  { key: 'task_assignment_id', label: 'Назначение', kind: 'number' },
  ...
  { key: 'is_overdue', label: 'Просрочен', kind: 'boolean' },
  { key: 'is_accepted', label: 'Принят', kind: 'boolean' },
];
...
export function ReportsByUserStatistics() {
  ...
  function updateFilters(patch: Partial<ReportsByUserPayload>) {
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MultiSearchSelect
            ...
            options={(regionsQuery.data ?? []).map((region) => ({ value: String(region.id), label: region.name, description: region.code }))}
          ...
          <SearchSelect
            ...
            onChange={(user_id) => updateFilters({ user_id: user_id ? Number(user_id) : null, page: 1 })}
          />
      ...
      {reportsMutation.isError && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-5 text-sm text-red-700">
          Не удалось загрузить статистику по пользователю.
        </div>
      )}

      {result ? (
        <ReportsByUserResult result={result} onPageChange={goToPage} />
      ) : (
...
function ReportsByUserResult({ result, onPageChange }: { result: ReportsByUserResponse; onPageChange: (page: number) => void }) {
  return (
...
function createInitialFilters(): ReportsByUserPayload {
  const now = new Date();
...
function SearchSelect({ label, value, placeholder, searchPlaceholder, options, onChange }: { label: string; value: string; placeholder: string; searchPlaceholder: string; options: SelectOption[]; onChange: (value: string) => void }) {
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent align="start" className="w-[min(520px,calc(100vw-3rem))] p-4">
          ...
          <ScrollArea className="mt-3 h-64 rounded-md border border-slate-200">
            <div className="p-1">
              <button type="button" className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-slate-100" onClick={() => { onChange(''); setOpen(false); }}>
                <Check className={cn('size-4 text-[#465cd3]', !value ? 'opacity-100' : 'opacity-0')} />
                <span className="font-medium text-slate-900">{placeholder}</span>
              </button>
              {filteredOptions.map((option) => (
                <button key={option.value} type="button" className="flex w-full items-start gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-slate-100" onClick={() => { onChange(option.value); setOpen(false); }}>
                  <Check className={cn('mt-0.5 size-4 text-[#465cd3]', value === option.value ? 'opacity-100' : 'opacity-0')} />
    ...
    <div className="space-y-1">
      ...
      <Popover>
        ...
        <PopoverContent align="start" className="w-[min(360px,calc(100vw-3rem))] p-2">
          <div className="space-y-1">
            {options.map((option) => (
              <button key={option.value} type="button" className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-slate-100" onClick={() => onChange(values.includes(option.value) ? values.filter((value) => value !== option.value) : [...values, option.value])}>
...
function TotalsGrid({ totals }: { totals: ReportsByUserResponse['totals'] }) {
  const items = [
    { label: 'Отчетов', value: totals.total_reports },
    { label: 'Принято', value: totals.accepted_reports },
    ...
    { label: 'Отправка отчетов', value: formatPercent(totals.submission_rate) },
...
function formatTableValue(item: ReportsByUserItem, column: (typeof tableColumns)[number]) {
...
function formatMetricValue(value: unknown, kind?: 'boolean' | 'date' | 'number') {
  if (value === null || value === undefined || value === '') return 'n/a';
  if (kind === 'boolean' && typeof value === 'boolean') return value ? 'Да' : 'Нет';
  if (kind === 'date' && typeof value === 'string') return formatDateTime(value);
  if (kind === 'number' && typeof value === 'number') return formatNumber(value);
  return String(value);
}

function formatAppliedFilters(filters: ReportsByUserResponse['filters_applied']) {
  return [
    ...
    { label: 'Пользователь', value: filters.user_id ?? 'Не выбран' },
...
function formatOptions(options: Array<{ value: string; label: string }>, values: string[]) {
  return values.length ? values.map((value) => getOptionLabel(options, value)).join(', ') : 'Все';
}
