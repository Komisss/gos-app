...
function createInitialFilters(): ReportsByUsersPayload {
  const now = new Date();
...
function TotalsGrid({ totals }: { totals: ReportsByUsersResponse['totals'] }) {
  const items = [
    { label: 'Исполнителей', value: totals.total_users },
...
function formatAppliedFilters(filters: ReportsByUsersResponse['filters_applied']) {
  return [
