...
const tableColumns: Array<{ key: keyof ReportsByDeadlinesItem; label: string; kind?: 'date' | 'number' }> = [
  { key: 'task_assignment_id', label: 'ID назначения', kind: 'number' },
  { key: 'task_id', label: 'ID задачи', kind: 'number' },
  { key: 'task_name', label: 'Задача' },
  { key: 'user_id', label: 'ID пользователя', kind: 'number' },
  { key: 'region_id', label: 'ID региона', kind: 'number' },
  { key: 'deadline_at', label: 'Дедлайн', kind: 'date' },
  { key: 'assignment_status', label: 'Статус назначения' },
  { key: 'has_report', label: 'Есть отчет' },
  ...
  { key: 'submitted_before_deadline', label: 'До дедлайна' },
  { key: 'submitted_after_deadline', label: 'После дедлайна' },
  { key: 'is_overdue', label: 'Просрочено' },
  { key: 'is_accepted', label: 'Принято' },
  { key: 'days_until_deadline', label: 'Дней до дедлайна', kind: 'number' },
  { key: 'delay_days', label: 'Дней задержки', kind: 'number' },
...
function ReportsDeadlinesResult({ result, onPageChange }: { result: ReportsByDeadlinesResponse; onPageChange: (page: number) => void }) {
  return (
...
function createInitialFilters(): ReportsByDeadlinesPayload {
  const now = new Date();
...
function TotalsGrid({ totals }: { totals: ReportsByDeadlinesResponse['totals'] }) {
  const items = [
    { label: 'Назначений', value: totals.total_assignments },
    { label: 'С дедлайном', value: totals.assignments_with_deadline },
    { label: 'Без дедлайна', value: totals.assignments_without_deadline },
    ...
    { label: 'Отправлено до дедлайна', value: totals.submitted_before_deadline },
    { label: 'Отправлено после дедлайна', value: totals.submitted_after_deadline },
    ...
    { label: 'Успешность дедлайна', value: formatPercent(totals.deadline_success_rate) },
    { label: 'Просрочки', value: formatPercent(totals.overdue_rate) },
    { label: 'Поздняя отправка', value: formatPercent(totals.late_submission_rate) },
    { label: 'Средняя задержка', value: totals.avg_delay_days },
    { label: 'Среднее выполнение', value: totals.avg_completion_time },
  ];
...
function formatTableValue(item: ReportsByDeadlinesItem, column: (typeof tableColumns)[number]) {
  const value = item[column.key];

  if (column.key === 'assignment_status') {
    return formatAssignmentStatus(value) ?? formatMetricValue(value, column.kind);
  }
...
function formatAppliedFilters(filters: ReportsByDeadlinesResponse['filters_applied']) {
  return [
