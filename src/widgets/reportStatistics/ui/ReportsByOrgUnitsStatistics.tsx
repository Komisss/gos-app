...
import { getReportsByOrgUnits } from '@/entities/analytics/api/dashboard';
import type {
  DashboardPeriodType,
  ReportAnalyticsAssignmentStatus,
  ReportAnalyticsTaskStatus,
  ReportsByOrgUnitsItem,
...
const tableColumns: Array<{ key: keyof ReportsByOrgUnitsItem; label: string; kind?: 'percent' | 'number' }> = [
...
export function ReportsByOrgUnitsStatistics() {
  const [filters, setFilters] = useState<ReportsByOrgUnitsPayload>(() => createInitialFilters());
  ...
  const tasksQuery = useQuery({
    queryKey: ['tasks', 'reports-by-org-units-filter'],
    queryFn: () => getTasks(),
  });
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MultiSelect
            ...
            onChange={(task_statuses) =>
              updateFilters({ task_statuses: task_statuses as ReportAnalyticsTaskStatus[], page: 1 })
            }
          />
          <MultiSelect
            label="Тип отчета"
        ...
        <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 md:grid-cols-2 xl:grid-cols-5">
          <BooleanFilter
            ...
            onChange={(include_child_org_units) => updateFilters({ include_child_org_units, page: 1 })}
          />
          <BooleanFilter
            label="Включать неактивные оргструктуры"
            checked={filters.include_inactive_org_units}
            onChange={(include_inactive_org_units) => updateFilters({ include_inactive_org_units, page: 1 })}
          />
          <BooleanFilter
...
function createInitialFilters(): ReportsByOrgUnitsPayload {
  const now = new Date();
...
function formatAppliedFilters(filters: ReportsByOrgUnitsResponse['filters_applied']) {
  return [
