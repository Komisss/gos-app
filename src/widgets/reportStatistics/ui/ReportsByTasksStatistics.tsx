...
const taskStatusOptions: Array<{ value: ReportAnalyticsTaskStatus; label: string }> = [
...
const tableColumns: Array<{ key: keyof ReportsByTasksItem; label: string; kind?: 'percent' | 'number' | 'date' }> = [
  ...
  { key: 'created_at', label: 'Создана', kind: 'date' },
  ...
  { key: 'revision_requests', label: 'Запросы правок', kind: 'number' },
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <FilterSelect
            ...
            onChange={(sort_by) => updateFilters({ sort_by: sort_by as ReportsByTasksPayload['sort_by'], page: 1 })}
          />
          <FilterSelect
            label="Направление"
      ...
      {reportsMutation.isError && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-5 text-sm text-red-700">
          Не удалось загрузить статистику по задачам.
        </div>
      )}
...
function createInitialFilters(): ReportsByTasksPayload {
  const now = new Date();
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent align="start" className="w-[min(520px,calc(100vw-3rem))] p-4">
          ...
          <ScrollArea className="mt-3 h-64 rounded-md border border-slate-200">
            <div className="p-1">
              {filteredOptions.length === 0 ? <div className="px-3 py-8 text-center text-sm text-slate-500">Ничего не найдено.</div> : filteredOptions.map((option) => (
...
function TotalsGrid({ totals }: { totals: ReportsByTasksResponse['totals'] }) {
  const items = [
    { label: 'Задач', value: totals.total_tasks },
...
function formatAppliedFilters(filters: ReportsByTasksResponse['filters_applied']) {
  return [
