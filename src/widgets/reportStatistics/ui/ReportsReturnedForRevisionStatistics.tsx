...
import { getReportsReturnedForRevision } from '@/entities/analytics/api/dashboard';
import type {
  ReportsReturnedForRevisionItem,
...
import { ReportsReturnedForRevisionExportPopover } from './ReportsReturnedForRevisionExportPopover';
...
const tableColumns: Array<{ key: keyof ReportsReturnedForRevisionItem; label: string; kind?: 'date' | 'number' }> = [
  ...
  { key: 'reason_id', label: 'ID причины', kind: 'number' },
  { key: 'reason_for_revision', label: 'Причина доработки' },
  { key: 'completion_time', label: 'Время выполнения', kind: 'number' },
  { key: 'is_overdue', label: 'Просрочено' },
  { key: 'is_accepted', label: 'Принято' },
];

export function ReportsReturnedForRevisionStatistics() {
  const [filters, setFilters] = useState<ReportsReturnedForRevisionPayload>(() => createInitialFilters());
  ...
  const usersQuery = useQuery({ queryKey: ['users', 'reports-returned-for-revision-filter'], queryFn: () => getUsers() });
  const tasksQuery = useQuery({ queryKey: ['tasks', 'reports-returned-for-revision-filter'], queryFn: () => getTasks() });
  const reportsMutation = useMutation({ mutationFn: getReportsReturnedForRevision });
  ...
  function updateFilters(patch: Partial<ReportsReturnedForRevisionPayload>) {
    setFilters((current) => ({ ...current, ...patch }));
  }

  function applyFilters(patch: Partial<ReportsReturnedForRevisionPayload>) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap justify-end gap-2">
        <ReportsReturnedForRevisionExportPopover
      ...
      {reportsMutation.isError && <div className="rounded-lg border border-red-200 bg-red-50 p-5 text-sm text-red-700">Не удалось загрузить статистику по возвращенным на доработку отчетам.</div>}
      {result ? <ReportsReturnedForRevisionResult result={result} filters={filters} taskOptions={taskOptions} onFiltersChange={applyFilters} onPageChange={goToPage} /> : <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">Настройте фильтры и нажмите «Получить статистику».</div>}
      <AnalyticsExportStatusToast
        exportJob={exportJob}
        title="Экспорт статистики по возвращенным на доработку"
        defaultFileName="analytics-returned-for-revision.xlsx"
...
function ReportsReturnedForRevisionResult({
  result,
  filters,
  taskOptions,
  onFiltersChange,
  ...
  result: ReportsReturnedForRevisionResponse;
  filters: ReportsReturnedForRevisionPayload;
  taskOptions: SelectOption[];
  onFiltersChange: (patch: Partial<ReportsReturnedForRevisionPayload>) => void;
              ...
              <TableHead className="min-w-[220px] align-top">
                <TableFilterCell>
                  <MultiSelect
                    ...
                    onChange={(report_types) => onFiltersChange({ report_types: report_types as ReportType[], page: 1 })}
                  />
                </TableFilterCell>
              </TableHead>
...
function createInitialFilters(): ReportsReturnedForRevisionPayload {
  const now = new Date();
...
function TotalsGrid({ totals }: { totals: ReportsReturnedForRevisionResponse['totals'] }) {
  const items = [
    ...
    { label: 'Доработки', value: formatPercent(totals.revision_rate) },
  ];
...
function formatTableValue(item: ReportsReturnedForRevisionItem, column: (typeof tableColumns)[number]) {
...
function formatAppliedFilters(filters: ReportsReturnedForRevisionResponse['filters_applied']) {
  return [
